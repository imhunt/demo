...
Set_config_password(){
	echo "请输入要设置的用户 密码"
	stty erase '^H' && read -p "(默认: 12345):" ssr_password
...
Set_config_method(){
 ...
 ${Green_font_prefix}1.${Font_color_suffix} 默认加密" && echo
	stty erase '^H' && read -p "(默认: 1. 默认加密方法):" ssr_method
	...
	echo && echo ${Separator_1} && echo -e "	加密 : " && echo ${Separator_1} && echo
}
Set_config_protocol(){
	echo -e "请选择要设置的用户 协议插件
 ${Green_font_prefix}1.${Font_color_suffix} 默认协议" && echo
	stty erase '^H' && read -p "(默认: 1. 默认协议):" ssr_protocol
	...
	echo && echo ${Separator_1} && echo -e "	协议 : " && echo ${Separator_1} && echo
	if [[ ${ssr_protocol} != "origin" ]]; then
		if [[ ${ssr_protocol} == "auth_sha1_v4" ]]; then
			stty erase '^H' && read -p "是否兼容原版？[Y/n]" ssr_protocol_yn
...
Set_config_obfs(){
	echo -e "请选择混淆插件
 ${Green_font_prefix}1.${Font_color_suffix} 默认插件" && echo
	stty erase '^H' && read -p "(默认: 1. 默认插件):" ssr_obfs
	...
	echo && echo ${Separator_1} && echo -e "	混淆 : " && echo ${Separator_1} && echo
	if [[ ${ssr_obfs} != "plain" ]]; then
			stty erase '^H' && read -p "是否设置兼容？[Y/n]" ssr_obfs_yn
...
Set_config_protocol_param(){
	...
	do
	echo -e "设备数限制"
	echo -e "${Tip} 设备数限制：建议最少 2个。"
	stty erase '^H' && read -p "(默认: 2个):" ssr_protocol_param
...
Set_config_speed_limit_per_con(){
	while true
	do
	echo -e "单程序限速(单位：KB/S)"
	echo -e "${Tip} 单程序的限速。"
	stty erase '^H' && read -p "(默认: 500k/s):" ssr_speed_limit_per_con
	[[ -z "$ssr_speed_limit_per_con" ]] && ssr_speed_limit_per_con=500 && echo && break
	expr ${ssr_speed_limit_per_con} + 0 &>/dev/null
	if [[ $? == 0 ]]; then
		if [[ ${ssr_speed_limit_per_con} -ge 1 ]] && [[ ${ssr_speed_limit_per_con} -le 102400 ]]; then
			echo && echo ${Separator_1} && echo -e "	单程序限速 : ${Green_font_prefix}${ssr_speed_limit_per_con} KB/S${Font_color_suffix}" && echo ${Separator_1} && echo
			break
		else
			echo -e "${Error} 请输入正确的数字(1-102400)"
		fi
	else
		echo -e "${Error} 请输入正确的数字(1-102400)"
	fi
...
Set_config_speed_limit_per_user(){
	...
	echo -e "总限速(单位：KB/S)"
	echo -e "${Tip} 总限速。"
	stty erase '^H' && read -p "(默认: 1000k/s):" ssr_speed_limit_per_user
	[[ -z "$ssr_speed_limit_per_user" ]] && ssr_speed_limit_per_user=500 && echo && break
	expr ${ssr_speed_limit_per_user} + 0 &>/dev/null
	if [[ $? == 0 ]]; then
		if [[ ${ssr_speed_limit_per_user} -ge 1 ]] && [[ ${ssr_speed_limit_per_user} -le 102400 ]]; then
			echo && echo ${Separator_1} && echo -e "	总限速 : ${Green_font_prefix}${ssr_speed_limit_per_user} KB/S${Font_color_suffix}" && echo ${Separator_1} && echo
			break
		else
			echo -e "${Error} 请输入正确的数字(1-102400)"
		fi
...
Set_config_transfer(){
	...
	echo
	echo -e "可用总流量(单位: GB)"
	stty erase '^H' && read -p "(默认: 100G):" ssr_transfer
	[[ -z "$ssr_transfer" ]] && ssr_transfer="100" && echo && break
	expr ${ssr_transfer} + 0 &>/dev/null
	if [[ $? == 0 ]]; then
		if [[ ${ssr_transfer} -ge 1 ]] && [[ ${ssr_transfer} -le 1024 ]]; then
			echo && echo ${Separator_1} && echo -e "	可用总流量 : ${Green_font_prefix}${ssr_transfer} GB${Font_color_suffix}" && echo ${Separator_1} && echo
			break
		else
			echo -e "${Error} 请输入正确的数字（1-1024）"
		fi
	else
		echo -e "${Error} 请输入正确的数字（1-1024）"
	fi
...
Set_user_api_server_pub_addr(){
	...
	if [[ -z "${ssr_server_pub_addr}" ]]; then
		...
		if [[ ${ip} == "VPS_IP" ]]; then
			while true
			do
			stty erase '^H' && read -p "${Error} 获取失败，请输入IP" ssr_server_pub_addr
			if [[ -z "$ssr_server_pub_addr" ]]; then
				echo -e "${Error} 不能为空！"
			else
				break
...
Modify_config_password(){
	...
	if [[ -z "${match_edit}" ]]; then
		echo -e "${Error} 修改失败 ${Green_font_prefix}[端口: ${ssr_port}]${Font_color_suffix} " && exit 1
	else
		echo -e "${Info} 修改成功 ${Green_font_prefix}[端口: ${ssr_port}]${Font_color_suffix} "
	fi
}
...
Install_SSR(){
	check_root
	[[ -e ${ssr_folder} ]] && echo -e "${Error} 程序已存在，请检查( 如安装失败或者存在旧版本，请先卸载 ) !" && exit 1
...
Install_Libsodium(){
	if [[ -e ${Libsodiumr_file} ]]; then
		echo -e "${Error} 加密程序 已安装 , 是否覆盖安装(更新)？[y/N]"
	...
	[[ ! -e ${Libsodiumr_file} ]] && echo -e "${Error} 加密程序 安装失败 !" && exit 1
	echo && echo -e "${Info} 加密程序 安装成功 !" && echo
}
# 显示 连接信息
...
————— 修改 用户配置 —————
 ...
 ${Green_font_prefix}4.${Font_color_suffix}  修改 设备数限制
 ${Green_font_prefix}5.${Font_color_suffix}  修改 单程序限速
 ${Green_font_prefix}6.${Font_color_suffix}  修改 总限速
 ${Green_font_prefix}7.${Font_color_suffix} 修改 总流量

 
 ${Tip} 用户的用户名和端口是无法修改的!" && echo
...
List_port_user(){
	...
	Get_User_transfer_all
	echo && echo -e "=== 账号总数 ${Green_background_prefix} "${user_total}" ${Font_color_suffix}"
	echo -e ${user_list_all}
	echo -e "=== 程序已使用流量总和: ${Green_background_prefix} ${transfer_enable_Used_233_2} ${Font_color_suffix}\n"
}
Add_port_user(){
	lalal=$1
...
Clear_transfer_all_cron_start(){
	...
	sed -i "/abc.sh/d" "$file/crontab.bak"
	echo -e "\n${Crontab_time} /bin/bash $file/abc.sh clearall" >> "$file/crontab.bak"
	crontab "$file/crontab.bak"
	rm -r "$file/crontab.bak"
	cron_config=$(crontab -l | grep "abc.sh")
...
Set_crontab_monitor_ssr(){
	SSR_installation_status
	crontab_monitor_ssr_status=$(crontab -l|grep "abc.sh monitor")
...
crontab_monitor_ssr_cron_start(){
	crontab -l > "$file/crontab.bak"
	sed -i "/abc.sh monitor/d" "$file/crontab.bak"
	echo -e "\n* * * * * /bin/bash $file/abc.sh monitor" >> "$file/crontab.bak"
	crontab "$file/crontab.bak"
	rm -r "$file/crontab.bak"
	cron_config=$(crontab -l | grep "abc.sh monitor")
	if [[ -z ${cron_config} ]]; then
...
Update_Shell(){
	...
	sh_new_ver=$(wget --no-check-certificate -qO- "https://softs.fun/Bash/abc.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="softs"
	[[ -z ${sh_new_ver} ]] && sh_new_ver=$(wget --no-check-certificate -qO- "https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/abc.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	if [[ ${sh_new_ver} != ${sh_ver} ]]; then
		...
		if [[ ${yn} == [Yy] ]]; then
			...
			if [[ $sh_new_type == "softs" ]]; then
				https://softs.fun/Bashwget -N --no-check-certificate https://softs.fun/Bash/abc.sh && chmod +x abc.sh
			else
				wget -N --no-check-certificate https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/abc.sh && chmod +x abc.sh
			fi
...
if [[ "${action}" == "clearall" ]]; then
	...
	echo -e "  我的脚本 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}

  ${Green_font_prefix}1.${Font_color_suffix} 安装程序 
  ${Green_font_prefix}2.${Font_color_suffix} 更新程序
  ${Green_font_prefix}3.${Font_color_suffix} 卸载程序
  ${Green_font_prefix}4.${Font_color_suffix} 加密程序
————————————
  ${Green_font_prefix}5.${Font_color_suffix} 查看 我的子账号
  ${Green_font_prefix}6.${Font_color_suffix} 显示 连接数据
  ${Green_font_prefix}7.${Font_color_suffix} 设置 子账号配置
  ${Green_font_prefix}8.${Font_color_suffix} 清零 已用流量
————————————
 ${Green_font_prefix}9.${Font_color_suffix} 启动程序
 ${Green_font_prefix}10.${Font_color_suffix} 停止程序
 ${Green_font_prefix}11.${Font_color_suffix} 重启程序
 ${Green_font_prefix}12.${Font_color_suffix} 查看日志
————————————
 "
	menu_status
